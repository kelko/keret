...
async fn list_entries<T: ToDoRepository>(State(repo): State<T>) -> Result<impl IntoResponse, StatusCode> {
    match repo.list() {
        Ok(list) => {
            Ok(Json(list))
        }
...
#[instrument(skip(repo))]
async fn add_entry<T: ToDoRepository>(State(repo): State<T>, Json(entry): Json<ActionReport>) -> Result<impl IntoResponse, StatusCode> {
